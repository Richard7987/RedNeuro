...
Anterior: [03 Preprocesamiento](03-preprocesamiento.md). Siguiente: [05 Gradient check](05-gradient-check.md). Índice: [RedNeuro: cómo se hizo](index.md).
...
El segundo importa. Un modelo baja de 0.693 a 0.306 **solo aprendiendo que los púlsares son raros**, sin mirar las features. Si la pérdida se estanca cerca de 0.3, la red aprendió el prior y nada más. En [06 Entrenamiento](06-entrenamiento.md) esa línea aparece como referencia gris.
...
Detalle: el notebook de Kaggle **nunca calcula la pérdida**, solo imprime accuracy. Aquí sí, porque quiero ver bajar el número que de verdad se optimiza, y porque accuracy no sirve con este desbalance ([07 Métricas con desbalance](07-metricas-con-desbalance.md)).
...
Ahí está la `X^T` de [Por qué los rangos dispares dañan el gradiente](03-preprocesamiento.md#por-qué-los-rangos-dispares-dañan-el-gradiente). Idéntico al original.
...
> **⚠️ Los shapes correctos no demuestran nada**
>
> Si escribes `Y − A2` en vez de `A2 − Y`, todos los shapes coinciden y la red sube la pérdida en vez de bajarla. Por eso el siguiente paso es el [05 Gradient check](05-gradient-check.md).
...
Qué pasa con `alpha` grande o chico está medido en [Experimentos con el learning rate](06-entrenamiento.md#experimentos-con-el-learning-rate).

---
title: 02 Datos HTRU2
---

# 02 Datos HTRU2

Anterior: [01 Entorno con Nix](01-entorno-con-nix.md). Siguiente: [03 Preprocesamiento](03-preprocesamiento.md). Índice: [RedNeuro: cómo se hizo](index.md).

Qué es el dataset y por qué se baja con un script está en el índice: [Qué es el dataset](index.md#qué-es-el-dataset). Aquí va lo que pasó al bajarlo y explorarlo.
...
Pandas lo lee sin problema porque reconoce `\r` como fin de línea. No hubo que convertir nada. Pero la lección quedó: cuando un archivo "se ve raro", `od -c | head` resuelve la duda en segundos. Más en [Fase 2](errores-y-lecciones.md#fase-2).

## Exploración

En `notebooks/01_explore.ipynb`. Dos tropiezos con el notebook en [Fase 2](errores-y-lecciones.md#fase-2): uno por escribir el `.ipynb` a mano como si fuera texto, otro por la ruta relativa.
...
Mirar la columna `std` de arriba a abajo. `ip_kurtosis` tiene desviación 1. `dm_skewness` tiene 106 y llega a 1191. Entre la feature más chica y la más grande hay un factor de cien. Eso es el problema que resuelve la normalización, y el porqué matemático está en [Por qué los rangos dispares dañan el gradiente](03-preprocesamiento.md#por-qué-los-rangos-dispares-dañan-el-gradiente).
...
`ip_kurtosis` e `ip_skewness` separan bastante bien las dos clases. Las `dm_` se solapan más. Eso adelanta qué features va a aprovechar la red, y después se ve confirmado en [08 Visualización en vivo](08-visualizacion-en-vivo.md): las líneas más gruesas salen de esas entradas.

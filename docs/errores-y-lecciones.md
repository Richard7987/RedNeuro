---
title: Errores y lecciones
---

# Errores y lecciones

Índice: [RedNeuro: cómo se hizo](index.md).
...
**`wc -l` dijo 0 líneas en un archivo de 17,898 filas, y `head -3` mostró una sola línea con basura al final.** El CSV de UCI usa `\r` como fin de línea, no `\n`. `wc -l` cuenta `\n`. `head -3` no encontró tres saltos y volcó el archivo entero, pero cada `\r` devuelve el cursor al inicio de la línea en la terminal y todas las filas se dibujaron una encima de otra. Se confirmó con `head -c 200 archivo | od -c`. Pandas lo lee bien; no hubo que tocar nada. Detalle en [El archivo raro](02-datos-htru2.md#el-archivo-raro).
...
Nada se rompió, pero hubo una **predicción equivocada** que vale registrar: esperaba que `alpha = 3.0` divergiera y no lo hizo, dio mejor resultado que 0.1. La explicación es la normalización: con la superficie de pérdida bien condicionada se pueden dar pasos grandes. Está en [Experimentos con el learning rate](06-entrenamiento.md#experimentos-con-el-learning-rate).
...
- Los shapes correctos no demuestran nada. El único error real de la red que este proyecto podría haber tenido (un signo o una transpuesta en backward) lo habría atrapado solo el [05 Gradient check](05-gradient-check.md), y por eso está antes de entrenar.
